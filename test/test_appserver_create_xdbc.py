...
class TestAppserverCreateXDBC(boostertest.BoosterTestCase):
    ...
    def test_create_xdbc_appserver_in_nonexistent_group_results_in_500(self):
        """ An appserver-create-xdbc should fail with 500 if group does not exist """
        params = self.params
        params['appserver-name'] = "xdbc-crunch"
        params['group-name'] = "there-is-no-such-group"
        self.teardown_appservers.append("xdbc-crunch")
        response, body = self.booster.request(params)
        err = response.get("x-booster-error", "none")
        self.assertEqual(response.status, 500)
        self.assertTrue(err.find("Error running action 'appserver-create-xdbc'. Error: No such group") > -1)
